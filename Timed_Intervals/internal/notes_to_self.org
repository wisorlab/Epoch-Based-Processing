...
2) "The number of bands is dependent on the frequency range and resolution of the data" I don't think this matches the code in autoscore.m.
...
3) "A band-pass filter from 10 to 40 Hz for the EMG signal is used to eliminate artefacts" Check this. Right now it is 4 to 40. 
...
I'm trying another .edf file just in case the one I was using happened to be wacky.

Try Will's idea of plotting delta power vs. EMG to see some clustering.  