...
for i=1:num_of_vars_to_scale 
	...
	first_added_zero_loc = header.records*header.samples(indices(i))+1
	%first_zero_loc = a(1);
	X = 0:1:first_added_zero_loc-2;
	V = record(1,1:first_added_zero_loc-1);
	%Xq = 0:1/scale_factor(indices(i)):first_added_zero_loc-2;
	Xq = linspace(0,first_added_zero_loc-2,size(record,2));

	%Xq=Xq(1:end-1);
	newlactatevec = interp1(X,V,Xq);
	size(newlactatevec)
	size(record)
	%record(indices(i),:) = newlactatevec;  %don't overwrite until I'm sure it's working
	%clear a X Xq V 
end
