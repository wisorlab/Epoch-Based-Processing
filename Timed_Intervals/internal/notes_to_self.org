...
What am I using to normalize the lactate signal?  The data in the delta figure 
should go to 100 during baseline (last 4 hours of light phase) since that is what 
I'm using to normalize it. Check this. 

For lactate simulation cut off the start of the simulation until 6pm of the first day.  
Check the Excel files to make sure they all start at the same time. 
