...
% Find the largest values in hdr.samples (this is the number of samples per 'duration' seconds)
 max_samples_per_epoch = max(hdr.samples(1:4));   %I'm leaving off EDFAnnotations because I don't care about interpolate
...
for i=1:num_of_vars_to_scale 
    first_added_zero_loc = hdr.records*hdr.samples(indices(i))+1;
