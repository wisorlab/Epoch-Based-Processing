 ...
 * Monday, July 21, 2014
 Notes from talk with Jonathan: 
 1) For the PCA approach, do noise reduction on the data first.  Do the same thing I do in PROCESSLBATCHMODE.m. That may 
 help the clustering to work.  THIS HELPED SOME, BUT I THINK A MACHINE LEARNING APPROACH WILL BE MORE EFFECTIVE.  
 2) Compare their EMG to ours: Jonathan says our EMG signal is integrating under the curve of EMG data for each 10-second 
 epoch.  What are they doing in the Gilmour paper? Is it an average or integrating like we are?  Look at the .edf file and 
 ask Will. 
 3) For the Bayesian approach, choose "random" segments that are a) between 10AM and 2PM if not sleep deprived and b) containing 
 at least 10% of each of 3 states.  
 4) Try to quantify how different the average lactate is from the .edf as compared to the .txt file. Make a plot or something. 
 5) Eventually, I'll need Will or someone to re-score files in 2,5,10 second epochs so I can auto-score them and compare.  
 6) Play with Support Vector Machines in matlab.  There is ample documentation.  Maybe also look at my 
 notes from the Machine Learning course.  
