...
13) Check that the code still runs for all strains using lactate as the signal.  There were a couple of bugs that came up when 
running the lactate model.  DBA now runs with lactate.  runs with AKRs too.    Runs with BL too.  
